...
#        # in Rails 3.2 there is no easy way to modify an existing validation.  We have to hack it out and replace it.
        ...
        validates_numericality_of :question_id, :if => Proc.new { |d| d.question_group_id.nil? && !d.new_record? }
        validates_numericality_of :question_group_id, :if => Proc.new { |d| d.question_id.nil? && !d.new_record?}
